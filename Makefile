...
menu:
	...
	echo "7 make profile SRC=f.c      - per-function cycle profile (f.c or a project dir)"
...
.PHONY: info menu select test web diff golden prg bench profile update_phony 
...
profile:
	test -n "$(SRC)" || (echo "usage: make profile SRC=<file.c | project dir>" && exit 1)
	node tools/profile6502.mjs $(SRC)
